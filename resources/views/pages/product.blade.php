...
<br>
        ...
        <div class="products-section">
            <div class="products-header">
                <div class="sort-tabs">
                    <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'newest']) }}" class="sort-link {{ request('sort_by') == 'newest' || !request('sort_by') ? 'active' : '' }}">Mới nhất</a>
                    <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'selling']) }}" class="sort-link {{ request('sort_by') == 'selling' ? 'active' : '' }}">Bán chạy</a>
                    <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'price_asc']) }}" class="sort-link {{ request('sort_by') == 'price_asc' ? 'active' : '' }}">Giá thấp đến cao</a>
                    <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'price_desc']) }}" class="sort-link {{ request('sort_by') == 'price_desc' ? 'active' : '' }}">Giá cao đến thấp</a>
                </div>
                
                <div class="products-title-section">
                    <p class="products-subtitle">Hiển thị {{ $all_sanpham->count() }} trên {{ $all_sanpham->total() }} kết quả</p>
                </div>
            </div>
            <div class="products-grid" style="grid-template-columns: repeat(4, 1fr);">

                @foreach ($all_sanpham as $key => $value)
                    <div class="product-card-new animate-fade-up" style="animation-delay: {{ $loop->index * 0.05 }}s;">
            ...
            {{ $all_sanpham->links('custom_pagination') }}
        </div>
    </div>
</div>
